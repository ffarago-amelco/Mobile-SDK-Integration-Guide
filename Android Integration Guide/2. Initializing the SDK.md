...
Kotlin is completely interoperable with Java Apps that are written in Kotlin can use the SDK. However, these apps should add the formentioned `compileOptions` and also the following object below:
TODO: app module's `build.gradle` file
  ```groovy
compileOptions {
    sourceCompatibility JavaVersion.VERSION_1_8
    targetCompatibility JavaVersion.VERSION_1_8
...
public class MainActivity extends AppCompatActivity implements OptimoveSuccessStateListener {
    ...
    //**Important Note**: If the object implementing the `OptimoveSuccessStateListener` is a component 
    //with a _"Lifecycle"_ (i.e. `Activity` or `Fragment`), **_always_** unregister that object 
    //at the `onStop()` callback to prevent memory leaks.
  ...
  public void onConfigurationSucceed(SdkRequiredPermission... missingPermissions) {
    //If required, ask for permissions at this stage. See Permissions section below.
